...
| 项 | 说明 |
|---|---|
| **C:\tmp 误删事故（已关闭 · 零影响）** | 猎虫席子代理清理时删除了 `C:\tmp`。**2026-08-30 用户确认：C 盘从未存放用户数据（用户工作目录为 `D:\tmp`，完好），删除内容不含用户工作产物，事故降级为零影响关闭**——无需恢复，`ctmp-recovery.md` 仅作归档保留。原"8/4–23 工作产物"的归因有误。预防纪律仍有效：清理只碰本会话自建路径，共享根目录禁 `rm -rf`；删除两段式——先移隔离区（如 `D:\tmp-quarantine\<日期>`）观察后再清 |
