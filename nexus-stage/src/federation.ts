...
/** Fallback federal anchor offset (null bounds): x = -GAP. */
export const FEDERAL_ANCHOR_GAP = 80
/** Gutter gap between the card field's right edge and the federal anchor
 *  (bounds.maxX + GUTTER) — keeps the short stubs off the card field. */
export const FEDERAL_GUTTER_GAP = 20
...
export function activityEdges(
  ...
  // Merge identical (team, peer) routes into ONE edge: N concurrent routes
  // over the same span would stack perfectly opaque duplicates. The label
  // counts them (×N) and reports the OLDEST route's age — the one the user
  // has been waiting on longest.
  const merged = new Map<string, { team: string; peer: string; startedAt: number; count: number }>()
  for (const row of inFlight) {
    if (!anchors.has(row.team)) continue
    const key = `${row.team}\u0000${row.peer}`
    const cur = merged.get(key)
    if (cur === undefined) merged.set(key, { team: row.team, peer: row.peer, startedAt: row.startedAt, count: 1 })
    else {
      cur.startedAt = Math.min(cur.startedAt, row.startedAt)
      cur.count += 1
    }
  }
  ...
  for (const g of merged.values()) {
    const anchor = anchors.get(g.team)
    if (anchor === undefined) continue
    // Match by parsed host:port — a bare endsWith would let 'badhost:port'
    // satisfy a 'host:port' route.
    const badge = peers.find(p => { try { return new URL(p.url).host === g.peer } catch { return false } })
    if (badge === undefined) continue
    const elapsed = Math.max(0, Math.round((now - g.startedAt) / 1000))
    out.push({
      x1: anchor.x,
      y1: anchor.y,
      x2: badge.x,
      y2: badge.y,
      label: `a2a_route · ${g.peer} · ${elapsed}s` + (g.count > 1 ? ` ×${g.count}` : ''),
    })
  }
  return out
}
...
export function federalEdges(
  ...
  const anchor = bounds !== null
    ? { x: bounds.maxX + FEDERAL_GUTTER_GAP, y: peers.length > 0 ? (peers[0]!.y + peers[peers.length - 1]!.y) / 2 : (bounds.minY + bounds.maxY) / 2 }
